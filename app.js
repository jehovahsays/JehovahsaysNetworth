...
(function() {
    ...
    if (currentPath.includes(pathFragmentToHide)) {
        ...
        const newPath = currentPath.replace(pathFragmentToHide, '/');
        const newURL = location.origin + newPath + location.hash;

        // Use replaceState to change the URL without reloading the page or creating a history entry.
        try {
            window.history.replaceState(null, null, newURL);
            console.log("✅ URL path successfully obscured (removed /en/).");
        } catch (e) {
            console.warn("Could not rewrite URL using History API (might be running locally or restricted):", e);
        }
    }
})();
...
async function showAbout() {
  ...
  c.innerHTML = `<section class="page"><h2>About</h2>
    ...
    <p>Source Version: 1.2.0-standalone</p>
    </section>`;
  location.hash = "#about";
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!cliActionHandled) {
    let initialHash = decodeURIComponent(location.hash.slice(1));
    
    // ✅ CRITICAL FIX: Handle the temporary 'loading' hash from index.html redirect
    if (initialHash === 'loading') {
        await showPage("Main-Page");
        // Update URL hash to #main after successful load
        location.hash = '#main';
        return; // Exit initialization logic
    }
